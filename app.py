...
@st.cache_resource
def load_model():
    model = joblib.load(r"C:\\Users\\moham\\OneDrive\\Documents\\COE-Heart dataset\\HeartData.pkl")
    return model

# Load the dataset to get feature names
@st.cache_resource
def load_data():
    df = pd.read_csv("C:\\Users\\moham\\OneDrive\\Documents\\COE-Heart dataset\\heart.csv.xls")
    return df
...
try:
    ...
    st.error(f"❌ Error: {e}")
    st.info("Make sure the model file is saved at: C:\\Users\\moham\\OneDrive\\Documents\\COE-Heart dataset\\HeartData.pkl")
